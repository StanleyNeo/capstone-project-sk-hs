...
// Replace just the school methods with this:
static async getSchools() {
  try {
    ...
    if (text.trim().startsWith('<!DOCTYPE') || text.trim().startsWith('<html')) {
      console.warn('Backend returned HTML instead of JSON. Using mock data.');
...
static async addSchool(schoolData) {
  console.warn('Using mock addSchool - Backend API might not be available');
  return {
    success: true,
...
static async updateSchool(id, schoolData) {
  console.warn('Using mock updateSchool - Backend API might not be available');
  return {
    success: true,
    message: 'School updated successfully (mock)'
  };
}

static async deleteSchool(id) {
  console.warn('Using mock deleteSchool - Backend API might not be available');
  return {
    success: true,
    message: 'School deleted successfully (mock)'
  };
}
