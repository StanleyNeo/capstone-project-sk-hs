...
// ========== ✅ HYBRID AI CHAT (uses FinalAIService — for frontend compatibility) ==========
app.post('/api/hybrid-ai/chat', async (req, res) => {
  try {
    ...
    const aiResponse = await FinalAIService.queryAI(message,
      "You are Athena, a supportive AI learning assistant. Respond in 1-2 short paragraphs. Be helpful and mention specific courses like 'Web Development Fundamentals' when relevant."
    );
    ...
    res.json({
      success: true,
      response: `👋 Hello! I'm Athena, your AI learning assistant. Try asking: 'What is React?' or 'Recommend a Python course.'`,
      provider: 'fallback'
    });
  }
});
